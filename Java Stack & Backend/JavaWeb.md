...
2021版：[JavaWeb视频教程，跟着老杜学javaweb零基础入门到精通_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Z3411C7NZ/?spm_id_from=333.337.search-card.all.click&vd_source=bf4c8577fb22820a80ba9f8a7e04169a)

> 学习目标：
>
> **适当的敲敲代码**,需要**对整个互联网的通信流程有一个大体的概念与认知**。里面一些技术栈比如servlet 、jsp虽然工作不会用到，但是时代技术方案的演变是基于这些经典的设计之上的。所以了解他们还是非常有必要的。适当的敲敲代码，脉络会更清楚。
>
> 课程评价：
>
> 对于开发有个提升层次的认知, 学了java语法天天在控制台sout..本地输出能干嘛?数据库能干嘛?这个课程就是打开你的开发思维,你会对互联网通信有一个宏观的认知和了解。从用户点击一个按钮。我们作为服务端的开发人员。我们可以干嘛?总之是一套打通任督二脉的东西。虽然jsp、servlet这些东西不用了，但是**了解一下经典的设计还是很有必要**的。
...
Servlet （Server applet）名字来源于“applet”的后缀“let”，意味着它是一个小程序或小组件，运行在服务器端而不是客户端。Servlet的主要功能包括：

1. **接收请求**：Servlet通过其`service`方法（或其他如`doGet`、`doPost`等具体方法）接收来自客户端（通常是Web浏览器）的HTTP请求。
2. **处理请求**：Servlet可以在服务器端执行业务逻辑，如查询数据库、处理表单数据等。
3. **生成响应**：Servlet生成动态内容，并将这些内容作为HTTP响应发送回客户端。
4. **会话管理**：Servlet可以管理用户会话，跟踪用户的状态和偏好。
5. **集成其他服务**：Servlet可以与后端服务（如数据库、邮件服务器等）集成，提供更复杂的功能。

Servlet通常作为Web应用程序的一部分运行在Web服务器或应用服务器上，如Apache Tomcat、JBoss、GlassFish等。这些服务器提供了Servlet运行所需的环境，包括请求分发、会话管理、安全等。
...
2. C/S架构？
   ...
   - C/S架构的软件或者说系统有哪些呢？
     - QQ（先去腾讯官网下载一个QQ软件，然后把这个客户端软件安装上去，然后输入QQ号以及密码，登录之后，就可以和你的朋友聊天了）
   - C/S架构的**特点：需要安装特定的客户端软件**。
   - C/S架构的系统优点和缺点分别是什么？
     - 优点：
       - **速度快**（软件中的数据大部分都是集成到客户端软件当中的，**很少量的数据从服务器端传送过来，所以C/S结构的系统速度快**）
       - 体验好（速度又快，界面又酷炫）
       - 界面酷炫（**专门的语言去实现界面**的，更加灵活）
       - **服务器压力小**（因为**大量的数据都是集成在客户端软件**当中，所以服务器只需要传送很少的数据量，当然服务器压力小。）
       - 安全（因为大量的数据是集成在客户端软件当中的，并且客户端有很多个，服务器虽然只有一个，就算服务器那边地震了，服务器受损了，问题也不大，因为**大量的数据在多个客户端上有缓存，有存储**，所以从这个方面来说，C/S结构的系统比较安全。）
       - .....
     - 缺点：
...
3. B/S架构？
   ...
   - B/S结构的系统优点和缺点是：
     - 优点：
       - **升级维护方便，成本比较低**。（**只需要升级服务器端**即可。）
       - 不需要安装特定的客户端软件，用户操作极其方便。**只需要打开浏览器，输入网址**即可。
     - 缺点：
       - **速度慢**（**不是因为带宽低**的问题，是因为**所有的数据都是在服务器上**，用户发送的每一个请求都是需要服务器全身心的响应数据，所以**B/S结构的系统在网络中传送的数据量比较大**。）
       - 体验差（界面不是那么酷炫，因为浏览器只支持三个语言HTML CSS JavaScript。再加上速度慢）
       - 不安全（**所有的数据都在服务器上，只要服务器发生事故**，地震等不可抗力，最终数据全部丢失。）
       - ....
4. C/S和B/S结构的系统，哪个好，哪个不好？
   - 这个问题问的没有水平。并不是哪个好，哪个不好。**不同结构的系统在不同的业务场景下有不同的适用场景**。
   - 娱乐性软件（比如QQ、音乐播放器、大型客户端游戏）建议使用？
     - C/S 结构
   - 公司内部使用的一些业务软件建议使用？
     - 公司内部使用的系统，需要**维护成本低**。
     - 公司内部使用的系统，不需要很酷炫。
...
5. 注意：**开发B/S结构的系统，其实就是开发网站，其实就是开发一个WEB系统**。
   - 开发一个WEB系统你需要会哪些技术？
     - WEB前端（运行在浏览器上的程序）
       ...
       - JavaScript
     - **WEB后端（WEB服务器端的程序**）
       - Java可以（**Java做WEB开发称为JavaWEB开发**。**JavaWEB开发最核心的规范：Servlet**【Server Applet服务器端的Java小程序】）
       - C语言也可以
       - C++也可以
       - Python也行
       - PHP也可以
...
6. JavaEE是什么？
   - Java包括三大块：
     ...
     - JavaEE（WEB方向，WEB系统）
       - **Java企业版**（也是一套类库：也是别人写好的一套类库，只不过**这套类库可以帮助我们完成企业级项目的开发**，专门为企业内部提供解决方案的一套（多套）类库。）
   ...
   - JavaEE实际上包括很多种规范，13种规范，其中**Servlet就是JavaEE规范之一**。学Servlet还是Java语言。
...
- 关于域名：
  ...
  - www.baidu.com 是一个域名
  - 在浏览器地址栏上输入域名，回车之后，**域名解析器会将域名解析出来一个具体的IP地址和端口号**等。
  - 解析结果也许是：http://110.242.68.3:80/index.html
- IP地址是啥？
  - 计算机在网络当中的一个身份证号。在**同一个网络当中，IP地址是唯一**的。
...
- 端口号是啥？
  ...
  - 在**同一个计算机上，端口号具有唯一性**。
- 一个WEB系统的通信原理？通信步骤：
  - 第一步：用户**输入网址**（URL，Uniform Resource Locator）
  - 第二步：域名解析器进行**域名解析**：http://110.242.68.3:80/index.html
  - 第三步：浏览器软件在网络中**搜索110.242.68.3这一台主机**，直到找到这台主机。
  - 第四步：定位110.242.68.3这台主机上的服务器软件，因为是80端口，可以很轻松的**定位到80端口对应的服务器软件**。
  - 第五步：80端口对应的**服务器软件得知浏览器想要的资源名**是：index.html
  - 第六步：**服务器软件找到**index.html文件，并且**将index.html文件中的内容直接输出响应到浏览器**上。
...
- 什么是请求，什么是响应？
  - 请求和响应实际上说的是数据的流向不同。
  - 从**Browser端发送数据到Server端，称为请求（request**）
  - 从**Server端向浏览器Browser端发送数据，称为响应（response**）
  - B --> S （请求request）
  - S --> B （响应response）

![](https://gitee.com/HYHZRYX/images/raw/master/imgs/202410121445666.png)



## 关于WEB服务器软件

- WEB服务器软件都有哪些呢？（这些软件都是别人提前开发好的）
  - **Tomcat（WEB服务器）**
  - **jetty（WEB服务器）**
  - JBOSS（应用服务器）
  - WebLogic（应用服务器）
...
- 应用服务器和WEB服务器的关系？
  ...
  - 应用服务器是包含WEB服务器的。
  - 用过JBOSS服务器的同学清楚，JBOSS中内嵌了一个Tomcat服务器。
...
- Tomcat下载
  ...
  - **tomcat是Java语言写的**。
  - tomcat服务器**要想运行，必须先有jre**（Java的运行时环境）
...
- Tomcat服务器的安装：
  ...
  - 启动Tomcat
    ...
    - > 如果在Path中没配置%CATALINA_HOME%\bin，DOS命令中就不会知道startup.bat在哪，也就不会执行；除非是在tomcat的目录中运行该bat
    
    - 启动Tomcat服务器只配置path对应的bin目录是不行的。有两个环境变量需要配置：
      - JAVA_HOME=JDK的根
      - CATALINA_HOME=Tomcat服务器的根 (D:\apache-tomcat-11.0.0)
...
- ```bat
	startup.bat   # 正规写法，因为bin目录里的文件名就叫startup.bat
	startup    # 也可以省略.bat
	```

- 关闭Tomcat：   (不要在DOS窗口直接点 “×” 关闭，要通过命令来关闭)

- ```bat
	shutdown.bat     # 这里不要省略.bat，因为shutdown是windows自己的关机命令, 二者可能冲突
	# 如果怕命令输错，可以在tomcat的bin目录下将shutdown.bat 重命名为 stop.bat （或其他不会冲突的名字）
	stop
	stop.bat
	```

- 怎么测试本机的Tomcat服务器有没有启动成功呢？
  - 打开浏览器，在浏览器的地址栏上输入URL即可：
    ...
    - ```sh
    	http://127.0.0.1:8080   # 127.0.0.1 就是本机IP
    	http://localhost:8080
    	```



## 实现一个最基本的web应用（无小Java程序）

- 第一步：找到%CATALINA_HOME%\webapps目录
  
  - 因为所有的webapp要放到webapps目录下。（这是Tomcat服务器的要求。如果不放到这里，Tomcat服务器就找不到你的应用）
- 第二步：在%CATALINA_HOME%\webapps目录下新建一个子目录，起名：oa  （项目名）
...
- 思考一个问题：
  ...
  - 我们在浏览器上直接输入一个URL，然后回车。这个动作和超链接一样吗？既然是一样的，我们完全可以使用超链接。
    ...
    ```html
    <!-- 用户点击超链接相当于在浏览器地址栏上输入URL （超链接中的IP地址和端口号可以省略）-->
    <!--注意以下的路径，以/开始，带项目名，是一个绝对路径。（也可以使用相对路径，但这里使用绝对路径）-->
    <a href="/oa/login.html">user login2</a>   <!-- 相当于输入http://127.0.0.1:8080/oa/login.html-->
    
    <!--多个层级也没有关系，正常访问即可。-->
    <!--注意：我们目前 前端上的路径都以“/”开始的，都是加项目名的。-->
...
## 对于一个动态的web应用，一个请求和响应的过程有多少角色参与，角色之间有多少协议

<img src="https://gitee.com/HYHZRYX/images/raw/master/imgs/202410121703829.png"  />

- 有哪些角色：（在整个BS结构的系统当中，有哪些人参与进去了）
  - Browser浏览器软件的开发团队    （谷歌浏览器、火狐浏览器、Edge浏览器....）
...
- 角色和角色之间需要遵守哪些规范，哪些协议：
  - webapp的开发团队   和    WEB Server的开发团队  之间有一套规范:   JavaEE规范之一 ，Servlet规范。
    - Servlet规范的作用是什么？
      - **WEB Server   和   webapp解耦合**。
  - Browser  和   WebServer 之间有一套传输协议：HTTP协议。（超文本传输协议）
  - webapp开发团队  和  DB Server的开发团队之间有一套规范：JDBC规范。

![](https://gitee.com/HYHZRYX/images/raw/master/imgs/202410121716028.png)

- Servlet规范是一个什么规范？  （具体在JavaEE帮助文档中查看）
  - **遵循Servlet规范的webapp可以放在不同的WEB Server中运行**。（因为这个webapp是遵循Servlet规范的）
  - Servlet规范包括什么呢？
    - 规范了哪些接口
    - 规范了哪些类
...
- 对于我们JavaWeb程序员来说，我们只需要做两件事：
	...
	> 注意：这个配置文件的文件名不能乱来，是固定的。 这个配置文件的存放路径不能乱来，是固定的。
...
## 开发一个带有Servlet(小Java程序)的webapp (重点）
...
- 开发步骤是怎样的？
  ...
  - 第二步：在webapp的**根下新建一个目录：WEB-INF**

    - 注意：这个目录的名字是Servlet规范中规定的，**必须**全部大写，必须一模一样。

  - 第三步：在WEB-INF目录下**新建一个目录：classes**

    - 注意：这个目录的名字**必须**是全部小写的classes。这也是Servlet规范中规定的。另外**这个目录下一定存放的是Java程序编译之后的class文件**（这里存放的是字节码文件）。

  - 第四步：在WEB-INF目录下**新建一个目录：lib**

    - 注意：这个目录**不是必须**的。但**如果一个webapp需要第三方的jar包的话，这个jar包要放到这个lib目录下**，这个目录的名字也不能随意编写，必须是全部小写的lib。   例如java语言连接数据库需要数据库的驱动jar包。那么这个jar包就一定要放到lib目录下。这Servlet规范中规定的。

  - 第五步：在WEB-INF目录下**新建一个文件：web.xml**

    - 注意：这个文件是**必须**的，这个文件名必须叫做web.xml。这个文件必须放在这里。一个合法的webapp，web.xml文件是必须的，这个**web.xml文件就是一个配置文件，在这个配置文件中描述了请求路径和Servlet类之间的对照关系**。
  ...
  - 第六步：编写一个Java程序，这个小Java程序也不能随意开发，这个小Java程序必须实现Servlet接口。
    ...
    - 这个Servlet**接口不在JDK**当中。（因为Servlet不是JavaSE了。**Servlet属于JavaEE，是另外的一套类库**。）
    - **Servlet接口（Servlet.class文件）是Oracle提供**的。（最原始的是sun公司提供的。）
    - Servlet接口是JavaEE（现在叫Jakarta EE）的规范中的一员。
    - Tomcat服务器实现了Servlet规范，所以Tomcat服务器也需要使用Servlet接口。Tomcat服务器中应该有这个接口，Tomcat服务器的%CATALINA_HOME%\lib目录下有一个servlet-api.jar，解压这个servlet-api.jar之后，会看到里面有一个Servlet.class文件。
    - 重点：**从JakartaEE9开始**，**Servlet接口的全名变**为：**jakarta.servlet.Servlet**
    - 注意：编写这个Java小程序的时候，**Java源代码愿意在哪里就在哪里**，位置无所谓，你**只需要将java源代码编译之后的class文件放到WEB-INF\classes目录下即可**。
  ...
  - 第七步：编译我们编写的HelloServlet
    ...
    - 重点：你怎么让你的HelloServlet编译通过呢？  配置环境变量CLASSPATH
  
      CLASSPATH=.;C:\dev\apache-tomcat-10.0.12\lib\servlet-api.jar
  
    - 思考问题：以上配置的CLASSPATH和Tomcat服务器运行有没有关系？
  
      - 没有任何关系，以上**配置这个环境变量只是为了让你的HelloServlet能够正常编译生成class文件**。
  
  - 第八步：将以上编译之后的HelloServlet**.class文件拷贝到WEB-INF\classes目录**下。 (如果有包名目录包裹，就把整个拷贝过来; 包名一样的话，只放.class就行)
    ...
    - ```xml
      ...
      <web-app xmlns="https://jakarta.ee/xml/ns/jakartaee"
        ...
        metadata-complete="true">
      	...
      	<!--servlet映射信息-->
      	<servlet-mapping>
      		<!--这个也是随便的，不过这里写的内容要和上面的servlet-name一样。-->
      		<servlet-name>fdsafdsagfdsafdsa</servlet-name>
      		<!--这里需要一个路径-->
      		<!--这个路径唯一的要求是必须以 "/" 开始-->
      		<!--当前这个路径可以随便写-->
      		<url-pattern>/fdsa/fd/saf/d/sa/fd/sa/fd</url-pattern>
              <!-- Servlet规范要求URL模式必须以/开头，以表示它们是Web应用程序上下文根的子路径 ; 
                   不应以 / 结尾，除非它是一个精确匹配的路径。例如，/admin/ 实际上是一个路径前缀，而不是一个目录-->
  ...
  - 第十一步：打开浏览器，在浏览器地址栏上输入一个url，这个URL必须是：
    ...
    - 注意：浏览器上的请求路径和web.xml文件中的url-pattern的**唯一区别就是：浏览器上的请求路径带项目名**：/crm
  
  - 浏览器上编写的路径太复杂，可以在html中使用超链接。（**非常重要：html页面只能放到WEB-INF目录外面。**）
  
  - 以后不需要我们编写main方法了。**tomcat服务器负责调用main方法**，Tomcat服务器启动的时候执行的就是main方法。我们**javaweb程序员只需要编写Servlet接口的实现类，然后将其注册到web.xml文件中**，即可。
  ...
  - 浏览器发送请求，到最终服务器调用Servlet中的方法，是怎样的一个过程？（以下过程描述的很粗糙，其中还有很多步骤省略了）
    ...
    - Tomcat**服务器调用**com.bjpowernode.servlet.**HelloServlet对象的service方法**。



## 关于JavaEE的版本

- JavaEE目前最高版本是 JavaEE8  （2017年，之后的版本不叫JavaEE）   （不是JDK）

- 2017年，JavaEE被Oracle捐献了，Oracle将JavaEE规范捐献给 Eclipse 基金会了。

- 于是自2018年把JavaEE换名了，**以后不叫JavaEE了，以后都叫做 jakarta EE**。

- > Java语言的名称最初是由Java岛上盛产的咖啡联想到的，而雅加达（Jakarta）是印度尼西亚的首都，历史上也是咖啡贸易的重要港口，因此被投票选出该名字。

- JavaEE8版本升级之后的"JavaEE 9"，不再是"JavaEE9"这个名字了，叫做JakartaEE9

- > 版本发布：
	>
	> - **Jakarta EE 9**：于 2020 年 11 月发布，没有加入新功能，是 Eclipse Foundation 的首个正式版本。命名空间从 `javax.*` 迁移到 `jakarta.*`，所有模块的大版本号+1，如 Servlet 4.0.2 变为 Servlet 5，以表示其断层式升级。
	> - **Jakarta EE 9.1**：于 2021 年 6 月发布，相较于 9 没有加入新 API，主要提供对 Java SE 11 的运行支持。
	> - **Jakarta EE 10**：于 2022 年 9 月 22 日发布，推动了云原生 Java 技术的未来。

- **JavaEE8的时候对应的Servlet类名是：javax.servlet.Servlet**

- **JakartaEE9的时候对应的Servlet类名是：jakarta.servlet.Servlet** （包名都换了）

- 如果你之前的项目还是在使用javax.servlet.Servlet，那么你的项目无法直接部署到Tomcat10+版本上。你只能部署到Tomcat9-版本上。在Tomcat9以及Tomcat9之前的版本中还是能够识别javax.servlet这个包。  （tomcat有提供migration工具）

- 查看帮助文档：去Tomcat官网下载Full documentation，打开servletapi\index.html 即可查看。



## 解决Tomcat服务器在DOS命令窗口中的乱码问题（控制台乱码）

因为Tomcat的日志和控制台输出使用的是UTF-8编码，对于中文电脑的控制台默认通常是GBK编码。（如果系统语言的高级设置为非中文统一是unicode编码就不会有乱码）

将%CATALINA_HOME%/conf/logging.properties文件中的内容修改如下：

```sh
java.util.logging.ConsoleHandler.encoding = GBK   # 让tomcat控制台显示为GBK编码
```
...
public class HelloServlet implements Servlet{
    public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException{
        // 向 Tomcat 控制台打印输出
	    System.out.println("My First Servlet，HHelloServlet");
        
        // 设置响应的内容类型是普通文本或html代码 （这个设置类型需要在获取流对象之前才有效）
        ...
        out.print("<h1>hello servlet!</h1>");   // 会响应给浏览器
    }
}
```
...
- Servlet是Java程序，所以**在Servlet中完全可以编写JDBC代码连接数据库**。
...
- 集成开发工具很多，其中目前使用比较多的是：
  ...
  - IntelliJ IDEA（这个居多，IDEA在提示功能方面要强于Eclipse，也就是说IDEA使用起来比Eclipse更加智能，更好用）
...
- 使用IDEA集成开发工具开发Servlet
  ...
  - 第二步：新建模块（File --> new --> Module...）
    - 这里**新建的是一个普通的JavaSE模块**（这里先不要新建Java Enterprise模块，因为IDEA的新建JavaEE借助了Maven）
    - 这个Module自动会被放在javaweb的project下面。
    - 给这个Module起名：servlet01
    
  - 第三步：**让Module变成JavaEE的模块**。（让Module变成webapp的模块。符合webapp规范，即符合Servlet规范的Module）
    ...
    - **注意：在IDEA工具中根据Web Application模板生成的目录中有一个web目录，这个目录就代表webapp的根** （对应手动在tomcat的webapps中创建的项目oa）
    
  - 第四步（非必须）：根据Web Application生成的资源中有index.jsp文件，这里我选择删除这个index.jsp文件。
  
  - 第五步：**编写Servlet 实现类** （比如 StudentServlet）
    
    - 在 src 中 新建package，然后新建 Java Class
    - ```java
    	class StudentServlet implements Servlet   // 但此时会显示Servlet.class文件不存在
    	```
    - 怎么办？**将 %CATALINA_HOME%/lib/servlet-api.jar 和 jsp-api.jar 添加到 IDEA 的 classpath当中**
      - File --> Project Structrue --> Modules --> 选中当前模块，切换到 dependencies， 点击 “+”  --> Add JARS....  (选择Add Libraries 将 Tomcat 全加进来也可以，但这样引入的就是Tomcat的lib中的所有jar包)
    - 实现`jakarta.servlet.Servlet`接口中的5个方法。   （IDEA中使用 alt + enter 快速生成）
    - ```java
    	import jakarta.servlet.Servlet;
    	public class StudentServlet implements Servlet {}
    	```
    
  - 第六步：在Servlet实现类当中的**service()方法中编写业务代码**     （这里为了演示进行了JDBC连接数据库）
  
  - ```java
  	@Override
  	public void service(ServletRequest request, ServletResponse response) {
  	    			 // 默认变量名是servletRequest 与 servletResponse; 可以改成request 与 response
  	}
  	```
  
  - 第七步：**在WEB-INF目录下新建一个子目录：lib**（这个目录名不能随意，必须是全部小写的lib），并且将连接数据库的驱动jar包放到lib目录下。
  
  - 第八步：**在web.xml文件中完成StudentServlet类的注册**。     （将请求路径和Servlet之间对应起来）
  ...
  <web-app xmlns="http://xmlns.jcp.org/xml/ns/javaee"
      ...
      <servlet-mapping>
          <servlet-name>studentServlet</servlet-name>
          <!-- 路径必须以 "/" 开头 （Servlet specification要求的）-->
  ...
  - 第九步：在web目录下 新建一个html页面，在HTML页面中编写一个超链接，用户点击这个超链接会发送请求，Tomcat会执行后台的StudentServlet。
  
    - 注意：student.html  这个文件不能放到WEB-INF目录里面，只能放到WEB-INF目录外面。
    ...
    - ```html
      ...
      <body>
          <!--这里路径必须有项目名，是 /xmm ，（不是JSP，所以无法动态获取，先写死。）-->  
  ...
  - 第十步：**让IDEA工具去关联Tomcat服务器**。关联的过程当中将webapp部署到Tomcat服务器当中。
    ...
    - 然后点击左上角加号，点击Tomcat Server --> local   （在本地添加Tomcat Server）
  
    - 在弹出的界面中设置服务器Server的参数（基本上不用动）
  
    	> 在Application server旁的Configure中选择Tomcat home并添加。 Name是给服务器起名；  Open browser表示是否服务器启动后自动打开浏览器；   VM options是指定虚拟机参数  ；On'Update'action 选择代码改动后服务器的操作 ； JRE选择使用的JDK ； 
  
    - 在当前窗口中有一个Deployment（点击这个用来部署webapp），继续点击 “+ ” → Artifact (构件) 完成部署。
  
    - 之后在Deployment页面修改 Application context为：`/xmm`  （即表示项目名，与上面HTML中路径的项目名一致）
  
    - 完成后点击Apply，之后IDEA会有一个Services窗口，其中展示Tomcat Server （服务器可以配置多个） 
  
  - 第十一步：启动Tomcat服务器
  
    - 右上角有绿色的箭头 run 按钮 ，或者绿色的 debug 按钮，可以采用debug的模式启动Tomcat服务器。
  
    	> 开发中建议使用 debug 模式启动 Tomcat。
...
> Servlet 对象 是指一个 Servlet 类的实例 （Servlet 是一个运行在服务器端的小程序，主要用于接收客户端请求（例如，来自 Web 浏览器的请求）、处理这些请求，并返回相应的响应给客户端。Servlet 遵循Jakarta EE规范）
...
- 什么是Servlet对象生命周期？
  ...
  - Servlet对象什么时候被销毁。
  - Servlet对象创建了几个。
  - **Servlet对象的生命周期表示：一个Servlet对象从出生在最后的死亡的整个过程**。

- Servlet对象是由谁来维护的？

  - Servlet对象的创建，对象上方法的调用，对象最终的销毁，**JavaWeb程序员是无权干预**的。
  - **Servlet对象的生命周期是由Tomcat服务器（WEB Server）全权负责的**。
  - **Web Server通常又称为：WEB容器（WEB Container**）
  - **WEB容器来管理Servlet对象的死活**。

- 思考：我们自己new的Servlet对象受WEB容器的管理吗？

  - **我们自己new的Servlet对象是不受WEB容器管理**的。
  ...
  - web容器底层应该有一个HashMap这样的集合，在这个**集合当中存储了Servlet对象和请求路径之间的关系**
  - <img src="https://gitee.com/HYHZRYX/images/raw/master/imgs/202410131458249.png" style="zoom: 80%;" />
...
- 怎么让服务器启动的时候创建Servlet对象呢？  （一般情况下不这样设置，因为没必要）
  ...
  - ```xml
    <servlet>
        ...
        <load-on-startup>1</load-on-startup>   <!-- 越小的整数优先级越高 （负数没必要，就用0和正整数） -->
    </servlet>
    <servlet-mapping>
        <servlet-name>aservlet</servlet-name>
...
- Servlet对象生命周期
  ...
  - 根据以上输出内容得出结论：
    ...
    - AServlet**对象被创建出来之后，Tomcat服务器马上调用了AServlet对象的init方法**。（init方法在执行的时候，AServlet对象已经存在了。已经被创建出来了。）
    - 用户**发送第一次请求**的时候，**init方法执行之后，Tomcat服务器马上调用AServlet对象的service方法**。
  ...
  - 根据以上输出结果得知，用户在发送第二次，或者第三次，或者第四次请求的时候，Servlet对象并没有新建，还是使用之前创建好的Servlet对象，直接调用该Servlet对象的service方法，这说明：
    ...
    - 1.：**Servlet对象是单例的**（单实例的。但是要注意：Servlet对象是单实例的，**但是Servlet类并不符合单例模式**。我们称之为**假单例**。之所以单例是因为Servlet对象的创建我们javaweb程序员管不着，这个对象的创建只能是Tomcat来说了算，Tomcat只创建了一个，所以导致了单例，但是属于假单例。真单例模式的构造方法是私有化的。）
    - 2.：**无参数构造方法 以及 init()方法只在第一次用户发送请求的时候执行**。也就是说无参数构造方法只执行一次。init方法也只被Tomcat服务器调用一次。
    - 3.：**只要用户发送一次请求，service()方法必然会被Tomcat服务器调用一次**。发送100次请求，service方法会被调用100次。
  ...
  - 通过以上输出内容，可以得出以下结论：
    ...
    - **Servlet的destroy()方法只被Tomcat服务器调用一次**。
    - destroy方法是在什么时候被调用的？
      - **在服务器关闭的时候**。
  ...
  - 当我们在Servlet类中编写一个有参数的构造方法，如果没有手动编写无参数构造方法会出现什么问题？
...
[10-Servlet对象的生命周期_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Z3411C7NZ/?p=11&spm_id_from=pageDriver&vd_source=bf4c8577fb22820a80ba9f8a7e04169a)
